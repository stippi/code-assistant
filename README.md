...
- ~~**Better search tool**: The current web search tool is very basic and doesn't work so well. Maybe an alternative tool using the Perplexity API would work better.~~ **Implemented!** The Perplexity API is now available as a search tool when the `PERPLEXITY_API_KEY` environment variable is set.
