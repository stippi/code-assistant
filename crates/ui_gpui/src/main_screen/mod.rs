...
pub enum MainScreenEvent {
    /// User clicked the settings gear icon.
    OpenSettings,
}
...
impl MainScreen {
    ...
    fn send_message(
        ...
        if let Some(sender) = cx.try_global::<UiEventSender>() {
            ...
            {
                tracing::warn!(
                    "MainScreen: Cannot send message — session {} is running in another instance",
                    session_id
                );
                return;
            }
            ...
            if branch_parent_id.is_some() {
                tracing::info!(
                    "MainScreen: Sending edited message (branch from {:?}) to session {}: {} (with {} attachments)",
                    branch_parent_id,
                    session_id,
                    content,
                    attachments.len()
            ...
            if agent_is_running {
                ...
                tracing::info!(
                    "MainScreen: Queuing user message for running agent in session {}: {} (with {} attachments)",
                    session_id,
                    content,
                    attachments.len()
                );
                ...
                tracing::info!(
                    "MainScreen: Sending user message to session {}: {} (with {} attachments, branch_parent: {:?})",
                    session_id,
                    content,
                    attachments.len(),
                    branch_parent_id
    ...
    fn show_new_project_dialog(
        ...
        let existing_projects = code_assistant_core::config::load_projects()
            .unwrap_or_default()
            .into_iter()
