...
fn process_chunked_text(text: &str, chunk_size: usize) -> TestUI {
    ...
    let mut processor = CaretStreamProcessor::new(ui_arc, 42, hidden_tools(), crate::tools::test_registry());
...
async fn test_caret_simple_tool() {
    let test_ui = TestUI::new();
    let ui_arc = Arc::new(test_ui.clone());
    let mut processor = CaretStreamProcessor::new(ui_arc, 123, hidden_tools(), crate::tools::test_registry());
