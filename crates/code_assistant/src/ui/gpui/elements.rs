...
pub enum ToolBlockState {
    /// Tool is being generated by LLM - show parameters but not output
    Generating,
    /// Tool is completed and collapsed - show parameters and output but collapsed
    Collapsed,
    /// Tool is completed and expanded - show all content expanded
    Expanded,
}
...
impl MessageContainer {
    ...
    pub fn add_tool_use_block(
        ...
        let block = BlockData::ToolUse(ToolUseBlock {
            ...
            status_message: None,
            output: None,
            state: ToolBlockState::Generating, // Default to generating
...
impl BlockView {
    ...
    fn toggle_tool_collapsed(&mut self, cx: &mut Context<Self>) {
        let should_expand = if let Some(tool) = self.block.as_tool_mut() {
            match tool.state {
                ToolBlockState::Generating => {
                    // Can't toggle while generating, ignore click
                    return;
                }
                    ...
                    .children(vec![
                        ...
                        div().flex_grow().min_w_0().relative().child(
                                ...
                                // Animated expandable content container
                                {
                                    let scale = match &self.animation_state {
                                        ...
                                        AnimationState::Idle => match block.state {
                                            ToolBlockState::Generating => 1.0, // Show content while generating
                                    ...
                                    let has_expandable_content = !fullwidth_params.is_empty() ||
                                        ...
                                        (block.state != ToolBlockState::Generating &&
                                                    ...
                                                    div()
                                                        ...
                                                        .children({
                                                            ...
                                                            if let Some(output_content) = &block.output {
                                                                if !output_content.is_empty() && block.state != ToolBlockState::Generating {
                                                                    let output_color =
                            ...
                            let (scale, is_expanding) = match &self.animation_state {
                                ...
                                AnimationState::Idle => match block.state {
                                    ToolBlockState::Generating => (1.0, false),
                            ...
                            if has_expandable_content && (block.state != ToolBlockState::Collapsed || scale > 0.0) {
                                ...
                                let footer_opacity = match &self.animation_state {
                                    AnimationState::Idle => match block.state {
                                        ...
                                        ToolBlockState::Generating | ToolBlockState::Expanded => 1.0,
                                    },
                                    AnimationState::Animating { .. } => {
                                        if is_expanding {
